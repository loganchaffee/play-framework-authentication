...
### java.lang.AssertionError: assertion failed: bad position: [241:239]
    ...
    request.headers.get("Authorization") match {
      ...
      case Some(tokenString) => {
        ...
        JWTService.verifyToken(token) match {
          case Left(_) => Unauthorized("Unauthorized")
          case Right(jwtPayload) => {
            // Fetch current user with jwtPayload.id if necessary
  ...
  def getMe() = Logging { request =>
    request.headers.get("Authorization") match {
      ...
      case Some(tokenString) => {
        ...
        JWTService.verifyToken(token) match {
          case Left(value) => Unauthorized("Unauthorized")
          case Right(jwtPayload) => {
            val user = userService.getUserById(jwtPayload.userId)
